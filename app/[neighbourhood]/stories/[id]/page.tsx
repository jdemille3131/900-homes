...
interface Props {
  params: Promise<{ neighbourhood: string; id: string }>;
}

export async function generateMetadata({ params }: Props) {
  const { id, neighbourhood: slug } = await params;
        ...
        <div className="mt-16 pt-12 border-t">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {(relatedStories as (Story & { story_media?: StoryMedia[] })[]).map((s) => (
              <StoryCard key={s.id} story={s} imageUrl={getImageUrl(s)} linkPrefix={prefix} />
            ))}
          </div>
